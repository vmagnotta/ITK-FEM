...
/**
 * \class LoadEdge
 * \brief A generic load that can be applied to an edge (face) of the element.
 *
 ...
 *
 * \sa {FEMLoadBC LoadBCMFC LoadGrav LoadLandmark LoadNode LoadPoint}
 */
class LoadEdge : public LoadElement
{
  FEM_CLASS(LoadEdge,LoadElement)
public:
  typedef vnl_matrix<Float>   LoadForceType;
   ...
   * This is the local number of the edge (face) of the element on which the load acts.
   ...
   * the force is applied.
   */ 
  void SetForce(LoadForceType force);
  
  /**
   * Get the force applied to the edge
   */ 
  LoadForceType GetForce() const;
